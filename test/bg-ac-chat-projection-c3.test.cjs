...
test('C3: exact-1.10 graph owns revision-bound chat execution projection', () => {
    assert.equal(adapter.version, '0.7.7')
...
test('C3: pre-canonical server input remains an explicit dormant contract', () => {
    ...
    assert.match(transform.content, /transformServerChatInput/)
    const inputTransform = unit('lazy-chat-bg-adapter:owned:server-chat-input-transform:1.10')
    assert.match(inputTransform.content, /triggers\.runTrigger/)
    assert.match(inputTransform.content, /scripts\.processScript/)
    assert.match(inputTransform.content, /character\.type === 'character'/)
