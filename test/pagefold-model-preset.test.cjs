...
test('prototype dependency and owned-file graph composes and reverts exactly', () => {
    ...
    assert.match(plan.outputs.get('src/ts/pagefold/qualifiedRoute.ts'), /never substitutes the v8 evidence model/)
    assert.doesNotMatch(plan.outputs.get('src/ts/pagefold/qualifiedRoute.ts'), /'unsupported-model'/)
    ...
    assert.match(plan.outputs.get('src/ts/preset/adapter/googleGemini.ts'), /applyPageFoldGeminiMediaResolution/)
    ...
    const pageFoldSettings = plan.outputs.get('src/lib/Setting/Pages/Model/PageFoldPresetSettings.svelte')
    assert.match(pageFoldSettings, /disabled=\{!modeReady\}/)
    assert.doesNotMatch(pageFoldSettings, /pageFoldQualified|pageFoldBlocked|pageFoldFidelity|ManualInputPrice|NumberInput/)
    ...
    const credentialField = plan.outputs.get('src/lib/Setting/Pages/Model/CredentialField.svelte')
    assert.match(credentialField, /planServiceAccountFileImport/)
    assert.match(credentialField, /notifySuccess\(language\.serviceAccountImportSuccess\)/)
    assert.match(credentialField, /isServiceAccountField && mode === 'direct'/)
    assert.match(credentialField, /className="w-full h-\[42px\]"/)
    assert.doesNotMatch(credentialField, /serviceAccountImportPlan|summary\.clientEmail|privateKeyId/)
    assert.match(plan.outputs.get('src/ts/preset/adapter/googleServiceAccount/serviceAccount.ts'), /projectIdRaw/)
    assert.match(plan.outputs.get('src/ts/pagefold/pricing.ts'), /record\('gemini-3\.7-flash', 0\.75, 3\.75/)
    assert.match(plan.outputs.get('src/ts/pagefold/pricing.ts'), /2027-01-01T00:00:00\.000Z/)
    assert.doesNotMatch(plan.outputs.get('src/ts/pagefold/pricing.ts'), /manual-pagefold|source: 'manual'/)
