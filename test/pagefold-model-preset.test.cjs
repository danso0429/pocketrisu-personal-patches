...
test('PageFold is exact-1.10 verified and admitted to the complete catalog', () => {
    ...
    assert.deepEqual(manifest.targets.pocketrisu, { verified: ['1.10.0'], reviewing: [] })
...
test('prototype dependency and owned-file graph composes and reverts exactly', () => {
    ...
    assert.match(plan.outputs.get('src/ts/process/request/request.ts'), /text: 'PF ON ' \+ pageFoldInfo!\.pdfPages \+ 'p'/)
    assert.doesNotMatch(plan.outputs.get('src/ts/process/request/request.ts'), /key: 'pagefold',[\s\S]{0,120}tone: 'success'/)
    const requestStatusToast = plan.outputs.get('src/lib/UI/GUI/RequestStatusToast.svelte')
    assert.match(requestStatusToast, /badge\.key === 'pagefold'/)
    assert.match(requestStatusToast, /<span class="rs-elapsed">\{badge\.text\}<\/span>/)
    assert.match(requestStatusToast, /badge\.key !== 'pagefold'/)
    ...
    assert.match(credentialField, /hasDirectKey && !isServiceAccountField/)
    const importButton = credentialField.indexOf('language.serviceAccountImportJson')
    const serviceAccountSave = credentialField.indexOf('{language.apiKeySave}', importButton)
    assert.ok(importButton >= 0 && serviceAccountSave > importButton)
    ...
    const generationInfoComponent = plan.outputs.get('src/lib/Others/PageFoldGenerationInfo.svelte')
    assert.match(generationInfoComponent, /\{signed\.toLocaleString\(\)\}/)
    assert.doesNotMatch(generationInfoComponent, /signed > 0 \? '\+' : ''/)
    assert.doesNotMatch(generationInfoComponent, /pageFoldGenerationPricing|pricingSource/)
    assert.match(plan.outputs.get('src/lang/en.ts'), /pageFoldGenerationDelta: "Saved tokens"/)
    assert.match(plan.outputs.get('src/lang/ko.ts'), /pageFoldGenerationDelta: "절약 토큰"/)
    const generationDialog = plan.outputs.get('src/lib/Others/AlertComp.svelte')
    assert.match(generationDialog, /generationInfoMenuIndex === 4/)
    assert.match(generationDialog, /generationInfoMenuIndex === 4 && !\$alertGenerationInfoStore\?\.genInfo\?\.pageFold/)
    assert.match(generationDialog, /\{language\.pageFoldTitle\}/)
    assert.match(generationDialog, /generationInfoMenuIndex === 4 && \$alertGenerationInfoStore\.genInfo\.pageFold/)
    assert.doesNotMatch(generationDialog, /tokenWarning\}<\/span>\n\s*\{#if \$alertGenerationInfoStore\.genInfo\.pageFold\}/)
