...
test('server fence covers storage and destructive recovery transitions but not reads or job creation', () => {
    ...
    assert.equal(writer('POST', '/api/import-jobs'), true)
    ...
    assert.equal(writer('DELETE', '/api/import-jobs/op-123456'), true)
    assert.equal(writer('GET', '/api/import-jobs/op-123456'), false)
