...
import { AppendableBuffer, BlankWriter, checkCharOrder, downloadFile, forageStorage, loadAsset, LocalWriter, readImage, requestImportedCharacterSave, saveAsset, VirtualWriter } from "./globalApi.svelte"
...
import { beginCharacterImport, formatCharacterImportProgress, type CharacterImportJob } from "./characterImportState"
...
type CharacterImportProcessOptions<T extends boolean> = {
    name: string;
    ...
    /** Parent package import owns its own blocking progress and final save. */
    suppressImportJob?:boolean
}

async function runCharacterImportJob<T>(
    ...
    try {
        ...
        const imported = (getDatabase().characters ?? []).filter(character =>
            typeof character?.chaId === 'string'
            && character.chaId.length > 0
            && !before.has(character.chaId)
        )
        ...
        job.update('Saving imported character...', 'Confirming the database update')
        for (const character of imported) {
            await requestImportedCharacterSave(character.chaId)
        }
        job.succeed(language.importedCharacter)
        return result
    } catch (error) {
        console.error('[Character Import]', error)
        job.fail(error)
        return null
    }
}

export async function importCharacterProcess<T extends boolean = false>(
    f: CharacterImportProcessOptions<T>,
):Promise<T extends true ? character | number | null : number | null>{
    if (f.returnCharacter || f.suppressImportJob) {
        return await importCharacterProcessInternal(f, null)
    }
    return await runCharacterImportJob(
        (job) => importCharacterProcessInternal(f, job),
    ) as T extends true ? character | number | null : number | null
}

async function importCharacterProcessInternal<T extends boolean = false>(
    f: CharacterImportProcessOptions<T>,
    importJob: CharacterImportJob | null,
):Promise<T extends true ? character | number | null : number | null>{
    if(f.name.endsWith('json')){
        ...
        if(await importCharacterCardSpec(da, undefined, 'normal', {}, null, false, importJob)){
            let db = getDatabase()
    ...
    if(f.name.endsWith('charx') || f.name.endsWith('jpg') || f.name.endsWith('jpeg')){
        ...
        if (importJob) importJob.update('Reading character archive...')
        else {
            alertStore.set({
                type: 'wait',
                msg: 'Loading... (Reading)'
        ...
        importer.alertInfo = !importJob
        if (importJob) {
            importer.setProgressHandler((done, total) => {
                importJob.update(formatCharacterImportProgress(
                    'Saving character assets...',
                    done,
                    total,
                ))
            })
    ...
    if (importJob) importJob.update('Reading character image...')
    else {
        alertStore.set({
            type: 'wait',
            msg: 'Loading... (Reading)'
            ...
            if(metaData.usePassword){
                ...
                if(!password){
                    ...
                    try {
                        ...
                        if(await importCharacterCardSpec(charaData, img, "normal", assets, null, false, importJob)){
                            let db = getDatabase()
                            return db.characters.length - 1
                        }
                        else{
    ...
    await importCharacterCardSpec(parsed, img, "normal", assets, null, false, importJob)
...
async function importCharacterCardSpec<T extends boolean = false>(
    ...
    importJob: CharacterImportJob | null = null,
    ...
    if (img && importJob) importJob.update('Saving character image...')
...
export async function downloadRisuHub(id:string, arg:{
    ...
    try {
        ...
        alertStore.set({ type: 'none', msg: '' })
        const imported = await runCharacterImportJob(async (job) =>
