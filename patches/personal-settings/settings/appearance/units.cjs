...
const units = [
    ...
    personalAppearanceFontPreview: "Multilingual font preview",
    personalAppearanceFontPreviewNote: "Noto covers the scripts shown here. Chat messages have no per-language tag, so shared Han characters use Korean glyph forms by default.",
...
`,
  ...
  personalAppearanceFontPreview: "다국어 폰트 미리보기",
  personalAppearanceFontPreviewNote: "Noto는 위 문자 범위를 지원해요. 실제 채팅에는 언어 태그가 없으므로 공용 한자는 기본적으로 한국어 자형을 사용해요.",
...
`,
    ...
    },
    {
        ...
        personalAppearanceChatFont: "Applies the selected font only to message bodies. Paperlogy is Korean-focused; Noto Sans KR and Noto Serif KR cover Korean, Japanese, Chinese, Latin, and extended Latin such as French. Shared Han characters default to Korean glyph forms because chat messages have no language tag. Code and keyboard input keep a monospace stack.",
...
`,
    ...
    },
    {
        ...
        "personalAppearanceChatFont": "선택한 폰트를 메시지 본문에만 적용해요. Paperlogy는 한글 중심이고, Noto Sans KR·Noto Serif KR은 한글·일본어·중국어·라틴 및 프랑스어 같은 확장 라틴을 지원해요. 채팅에는 언어 태그가 없으므로 공용 한자는 한국어 자형이 기본이에요. 코드와 키보드 입력은 고정폭 폰트를 유지해요.",
