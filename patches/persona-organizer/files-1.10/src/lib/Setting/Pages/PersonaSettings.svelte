...
<script lang="ts">
    ...
    function duplicatePersona(): void {
        const current = DBState.db.personas[DBState.db.selectedPersona]
        if (!current) return
        saveUserPersona()
        const clone = $state.snapshot(current)
        DBState.db.personas.push({
            ...clone,
...
>
    ...
    {#snippet children()}
        <div class="flex flex-col gap-2">
            ...
            <ShButton variant="outline" className="w-full justify-start" onclick={duplicatePersona}>
                {language.personaDuplicate}
            </ShButton>
        </div>
    {/snippet}
    {#snippet footer()}
