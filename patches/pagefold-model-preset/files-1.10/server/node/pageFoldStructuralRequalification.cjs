...
const PAID_OUTPUT_TOKENS_V4 = 2_048
...
const STRUCTURAL_EXPECTATION_V4 = Object.freeze({
    words: Object.freeze(['ALPHA', 'BETA']),
    spaceRunPositions: Object.freeze(['leading', 'between', 'trailing']),
    zwjSemanticMembers: Object.freeze(['man', 'woman', 'girl', 'boy']),
    zwjJoinerCount: 3,
    variationSequenceCodePoints: Object.freeze([9_992, 65_039]),
    tagSequenceCodePoints: Object.freeze([917_607]),
})
const STRUCTURAL_EXPECTATION_V5 = Object.freeze({
    words: Object.freeze(['ALPHA', 'BETA']),
    spaceRunPositions: Object.freeze(['leading', 'between', 'trailing']),
...
const STRUCTURAL_EXPECTATION_V8 = STRUCTURAL_EXPECTATION_V7
    ...
    if (finishReason === 'MAX_TOKENS') {
        return {
            ...
            outputControlAllowed: oracleVersion !== STRUCTURAL_ORACLE_V4
                ...
                && oracleVersion !== STRUCTURAL_ORACLE_V8
...
function expectedForClaim(claim, fixture, oracleVersion = STRUCTURAL_ORACLE_V1) {
    requireClaim(claim)
    requireOracleVersion(oracleVersion)
    const structuralExpectation = oracleVersion === STRUCTURAL_ORACLE_V8
    ...
    if (claim === 'page-markers') {
        return {
            markers: clone(oracleVersion === STRUCTURAL_ORACLE_V8
                ? fixture.markerBoundaries
                : oracleVersion === STRUCTURAL_ORACLE_V6
                    || oracleVersion === STRUCTURAL_ORACLE_V7
...
function responseSchemaForClaim(claim, oracleVersion = STRUCTURAL_ORACLE_V1) {
    ...
    if (claim === 'text-oracle') {
        ...
        if (oracleVersion === STRUCTURAL_ORACLE_V4) {
            return objectSchema(
                [
                    'words',
                    'spaceRunPositions',
                ...
                ],
                {
                    words: stringArray,
                    spaceRunPositions: stringArray,
                    zwjSemanticMembers: stringArray,
                    zwjJoinerCount: { type: 'integer' },
                    variationSequenceCodePoints: codePointArray,
                    tagSequenceCodePoints: codePointArray,
                    roles: rolePairArray,
    ...
    if (claim === 'page-markers') {
        if (oracleVersion === STRUCTURAL_ORACLE_V8) {
            const markerBoundary = objectSchema(['first', 'last'], {
                first: { type: 'string' },
                last: { type: 'string' },
            })
            return objectSchema(['markers'], {
                markers: { type: 'array', items: markerBoundary },
            })
        }
        if (oracleVersion === STRUCTURAL_ORACLE_V6
            ...
            return objectSchema(['markers'], {
                markers: { type: 'array', items: markerWindow },
            })
        }
        return objectSchema(['markers'], {
...
function promptForClaim(claim, oracleVersion = STRUCTURAL_ORACLE_V1) {
    ...
    if (claim === 'text-oracle') {
        if (oracleVersion === STRUCTURAL_ORACLE_V5
            ...
            return [
                ...
                'Keep words, whitespace positions, and role objects in listed order.',
                'Return variation/tag code points as base-10 JSON integer arrays.',
            ].join(' ')
        }
        if (oracleVersion === STRUCTURAL_ORACLE_V4) {
            return [
                'This is a response-schema control, not a visual perception test.',
                'Copy the already-computed labeled semantic facts into the matching JSON fields.',
                'Keep words, whitespace positions, family members, and role objects in listed order.',
                'Return variation/tag code points as base-10 JSON integer arrays.',
            ].join(' ')
        }
        if (oracleVersion === STRUCTURAL_ORACLE_V3) {
    ...
    if (claim === 'byte-structure') {
        if (oracleVersion === STRUCTURAL_ORACLE_V5
            ...
            return [
                ...
                'spaceRunPositions lists which of leading, between, and trailing contain a nonempty U+0020 run; do not estimate typographic run length.',
                'zwjSemanticKind is the single lowercase semantic kind of the rendered joined emoji; use family for a family emoji.',
                'zwjJoinerCount is the number of U+200D separators in that sequence.',
                'Return variationSequenceCodePoints and tagSequenceCodePoints as base-10 Unicode scalar values.',
            ].join(' ')
        }
        if (oracleVersion === STRUCTURAL_ORACLE_V4) {
            return [
                ...
                'Parse the canonical JSONL content and the visually rendered labeled sequences.',
                'spaceRunPositions lists which of leading, between, and trailing contain a nonempty U+0020 run; do not estimate typographic run length.',
                'zwjSemanticMembers lists the rendered family members in order using only the lowercase labels man, woman, girl, boy.',
                'zwjJoinerCount is the number of U+200D separators in that family sequence.',
                'Return variationSequenceCodePoints and tagSequenceCodePoints as base-10 Unicode scalar values.',
            ].join(' ')
        }
        if (oracleVersion === STRUCTURAL_ORACLE_V3) {
    ...
    if (claim === 'page-markers') {
        if (oracleVersion === STRUCTURAL_ORACLE_V8) {
            return 'For each physical PDF page return one object in page order with the exact first and exact last distinct Ldddddd code. Return every page and do not return center markers.'
        }
        if (oracleVersion === STRUCTURAL_ORACLE_V6
            || oracleVersion === STRUCTURAL_ORACLE_V7) {
...
function createTextControl(oracleVersion = STRUCTURAL_ORACLE_V1) {
    ...
    if (oracleVersion === STRUCTURAL_ORACLE_V4) {
        return [
            'PAGEFOLD_RESPONSE_ORACLE_V4',
            'WORDS|ALPHA|BETA',
            'SPACE_RUN_POSITIONS|leading|between|trailing',
            'ZWJ_SEMANTIC_MEMBERS|man|woman|girl|boy',
            'ZWJ_JOINER_COUNT|3',
            'VARIATION_SEQUENCE_SCALARS_DECIMAL|9992|65039',
            'TAG_SEQUENCE_SCALARS_DECIMAL|917607',
            'ROLE_OBJECTS|R_SYS|system|R_USER|user|R_ASSISTANT|assistant|R_TOOL|tool',
...
async function renderFixture({ renderer, pages, mode }) {
    ...
    const markerWindows = extracted.pages.map(markerWindow)
    const markerBoundaries = markerWindows.map(({ first, last }) => ({ first, last }))
    return {
        mode,
        pages,
        messageCount,
...
function markerTriple(page) {
    const codes = markerCodes(page)
    return [codes[0], codes[Math.floor(codes.length / 2)], codes[codes.length - 1]]
}

function markerWindow(page) {
    const codes = markerCodes(page)
    const lower = codes[Math.floor((codes.length - 1) / 2)]
    const upper = codes[Math.floor(codes.length / 2)]
    return {
        first: codes[0],
        centers: lower === upper ? [lower] : [lower, upper],
        last: codes[codes.length - 1],
    }
}

function markerCodes(page) {
    const codes = []
    const seen = new Set()
...
function publicDryRun(fixtures) {
    ...
    return {
        ...
        oracleVersions: {
            ...
            paidRunner: STRUCTURAL_ORACLE_V8,
        },
        ...
        paidOutputTokens: PAID_OUTPUT_TOKENS_V4,
        outputCapControlTokens: null,
        vertexRatedCostCapUsd: VERTEX_RATED_COST_CAP_USD,
        maximumCallsAfterApproval: screening.length + qualificationLow.length + hierarchyLow.length,
        screening,
        ...
        structuralExpectationV8: clone(STRUCTURAL_EXPECTATION_V8),
        responseOracleV2: {
        ...
        responseOracleV4: {
            control: createTextControl(STRUCTURAL_ORACLE_V4),
            prompt: promptForClaim('text-oracle', STRUCTURAL_ORACLE_V4),
            expected: expectedForClaim('text-oracle', {}, STRUCTURAL_ORACLE_V4),
            responseSchema: responseSchemaForClaim('text-oracle', STRUCTURAL_ORACLE_V4),
        },
        responseOracleV5: {
            control: createTextControl(STRUCTURAL_ORACLE_V5),
            prompt: promptForClaim('text-oracle', STRUCTURAL_ORACLE_V5),
            expected: expectedForClaim('text-oracle', {}, STRUCTURAL_ORACLE_V5),
            responseSchema: responseSchemaForClaim('text-oracle', STRUCTURAL_ORACLE_V5),
        },
        responseOracleV6: {
            control: createTextControl(STRUCTURAL_ORACLE_V6),
            prompt: promptForClaim('text-oracle', STRUCTURAL_ORACLE_V6),
        ...
        responseOracleV8: {
            control: createTextControl(STRUCTURAL_ORACLE_V8),
            prompt: promptForClaim('text-oracle', STRUCTURAL_ORACLE_V8),
            expected: expectedForClaim('text-oracle', {}, STRUCTURAL_ORACLE_V8),
            responseSchema: responseSchemaForClaim('text-oracle', STRUCTURAL_ORACLE_V8),
        },
        fixtures: Object.values(fixtures).map((fixture) => ({
            ...
            markerWindows: clone(fixture.markerWindows),
            markerBoundaries: clone(fixture.markerBoundaries),
...
function normalizeAnswerForComparison(claim, observed, oracleVersion) {
    if ((oracleVersion !== STRUCTURAL_ORACLE_V7
        && oracleVersion !== STRUCTURAL_ORACLE_V8) || !observed) return observed
