...
describe('StartupDatabaseCache probe and hydration', () => {
    it('uses a valid raw probe without waiting for a stalled decoded store', async () => {
        const namespace = createStartupDatabaseCacheNamespace('1.0.0', 1)
        const decoded = new MemoryDecodedStore()
        decoded.readMeta = () => new Promise(() => {})
        const raw = new MemoryRawStore(namespace)
        raw.etag = 'raw-etag'
        const cache = new StartupDatabaseCache({
            appVersion: '1.0.0',
        ...
        await expect(Promise.race([
            ...
            new Promise((_, reject) => setTimeout(
                () => reject(new Error('probe waited for stalled IndexedDB')),
                100,
            )),
        ])).resolves.toEqual({ etag: 'raw-etag', source: 'raw' })
    })

    it('uses a valid decoded probe without waiting for a stalled raw store', async () => {
        const namespace = createStartupDatabaseCacheNamespace('1.0.0', 1)
        const decoded = new MemoryDecodedStore()
        decoded.meta = baselineMeta(namespace, 'decoded-etag')
        const raw = new MemoryRawStore(namespace)
        raw.readMetadata = () => new Promise(() => {})
        const cache = new StartupDatabaseCache({
            appVersion: '1.0.0',
            schemaEpoch: 1,
            rawStore: raw,
        ...
        await expect(Promise.race([
            cache.probe(),
            new Promise((_, reject) => setTimeout(
                () => reject(new Error('probe waited for stalled CacheStorage')),
                100,
            )),
        ])).resolves.toEqual({ etag: 'decoded-etag', source: 'decoded' })
    })
