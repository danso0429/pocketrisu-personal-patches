...
export class StartupDatabaseCache {
    ...
    private async probeWithoutTimeout(): Promise<StartupDatabaseCacheProbe | null> {
        ...
        type ProbeResult =
            | { source: 'decoded', value: StartupDatabaseCacheMeta | null }
            | { source: 'raw', value: StartupRawCacheMetadata | null }

        const pending = new Map<'decoded' | 'raw', Promise<ProbeResult>>([
            ['decoded', this.decodedStore.readMeta()
                .then(value => ({ source: 'decoded' as const, value }))
                .catch(() => ({ source: 'decoded' as const, value: null }))],
            ['raw', this.rawStore.readMetadata()
                .then(value => ({ source: 'raw' as const, value }))
                .catch(() => ({ source: 'raw' as const, value: null }))],
        ])

        // Do not join the two browser stores. Mobile Safari can leave one
        // backend pending while the other already has a valid ETag. Returning
        // the first usable result avoids spending the whole startup timeout on
        // an optional cache backend.
        while (pending.size > 0) {
            const result = await Promise.race(pending.values())
            pending.delete(result.source)
            if (result.source === 'decoded') {
                if (result.value && this.isUsableMeta(result.value)) {
                    return { etag: result.value.currentEtag, source: 'decoded' }
                }
                if (result.value) void this.clearDecodedBestEffort()
                continue
            }
            if (result.value?.namespace === this.namespace && result.value.etag) {
                return { etag: result.value.etag, source: 'raw' }
            }
        }
        return null
    }
