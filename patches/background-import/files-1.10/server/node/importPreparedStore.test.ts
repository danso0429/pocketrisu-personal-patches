...
describe('private prepared import store', () => {
    ...
    test('root and operation symlinks never mutate their external targets', async () => {
        const { root, store } = await setup()
        const external = await fs.mkdtemp(path.join(os.tmpdir(), 'prepared-external-'))
        roots.push(external)
        await fs.chmod(external, 0o755)
        await fs.writeFile(path.join(external, 'preserve'), 'unchanged')
        await fs.symlink(external, store.stagingDir('prepared_symlink_001'))
        await expect(store.prepareStaging('prepared_symlink_001'))
            .rejects.toHaveProperty('code', 'IMPORT_PREPARED_PATH_INVALID')
        expect((await fs.stat(external)).mode & 0o777).toBe(0o755)
        expect(await fs.readdir(external)).toEqual(['preserve'])

        const linkedRoot = path.join(path.dirname(root), `linked-${path.basename(root)}`)
        await fs.symlink(external, linkedRoot)
        const linkedStore = createPreparedImportStore({ root: linkedRoot })
        await expect(linkedStore.prepareStaging('prepared_symlink_002'))
            .rejects.toHaveProperty('code', 'IMPORT_PREPARED_PATH_INVALID')
        expect((await fs.stat(external)).mode & 0o777).toBe(0o755)
        expect(await fs.readdir(external)).toEqual(['preserve'])
