...
| K23 | Regex and lorebook grouping/editing | V | M, X | Multi-type regex schema compatibility belongs to `bg-preserve`, which already owns `types[]` generation paths. Exclude Kei's single-type/broad UI rewrite unless a concrete missing outcome is specified and proved schema-compatible. |
