...
| P2 | Point-in-time server backup source | Automatic exact-1.9 and deterministic generated-installer qualification passed; live admission pending | Large SQLite-backed backups must be assembled from one pinned database/WAL and asset view. Without this, faster or detached backup execution can produce a logically mixed archive. |
