...
This worktree is branch `codex/pocketrisu-bg-independent`, originally formed at merge commit
...
| Predecessor root effects, conflict/retention join, browser-process exit | C1/C3 effect records, setting overlay, and root writer | Receipt-based value comparison exists, but an independent browser +1 and server N +1 can collapse during root-save rebase; root arbitration, joined retention, and browser/device gates remain. |
...
predecessor's own effect without additional identity; this is value
attestation, not writer provenance. The later root-race diagnostic in this
document did reproduce a harmful independent count increment. This historical
G1.4 checkpoint did not yet include that diagnostic. Blocked-edit recovery,
...
The input capability remains 0. Actual browser exit after result ACK,
root-save/effect arbitration, retention expiry, actual browser-process return,
cross-tab admission races, client-only normal-path preservation, and L2.5
remain open gates. The predecessor receipt/value comparison exists, but the
actual root-write protocol can discard an independent count effect. G1 is not
product-complete; G2/G3 have not started, and no live or AC source was changed.

## 2026-09-24 root-effect race diagnostic

The current implementation plan and fresh-session entry point are
`POCKETRISU-1.10-BG-INDEPENDENT-G1-ROOT-EFFECT-IMPLEMENTATION-PLAN.md` and
`POCKETRISU-1.10-BG-INDEPENDENT-G1-FRESH-START.md`. The replayable
exact-1.10 diagnostic patch is
...
Its five isolated spawned-server/SQLite/HTTP cases passed together. They
observed base count 10 plus one browser send and one N server effect:
browser-first stored 12; N-first patch-sync and conditional full-write
retries stored 11 with N's ledger intact; missing-ETag stale full write stored
10 with N's ledger intact. A committed browser patch with lost HTTP response
followed by N stored the correct remote count 12, while ordinary stale-client
rebase regressed it to 11; blind delta addition would produce 13. The
N-first patch case admitted N+1 from the incorrect count. Related existing
owner/context/projection tests passed 53/53 and client conflict/input/stats
...
preserve all effects. `inputCommandVersion=0` is insufficient as a rollout
...
recorded above. Follow the root-effect implementation plan R0–R6: map root
writers and retries, select an idempotent effect contract, change the patch
and full-write boundaries, and pass the adversarial order/response-loss
matrix before either commit or input capability is delivery-qualified. Then
close concurrent chat edits, cross-tab admission, joined
input/result/owner/context retention, production-bundle effects, and L2.5.
Verify the normal chat API **before** actual browser-process reopen.
