...
| Predecessor root effects, conflict/retention join, browser-process exit | C1/C3 effect records and setting overlay | Receipt-based value comparison exists. An independently durable, value-identical writer with a harmful outcome has not been reproduced; actual root-save races, joined retention, and browser/device gates remain. |
...
attestation, not cryptographic writer provenance. No independently durable,
value-identical writer causing a different user result has been reproduced;
this theoretical distinction alone does not establish a product defect or a
mandatory writer-ledger design. Blocked-edit recovery,
...
same-key root-save/effect races, retention expiry, actual browser-process return,
cross-tab admission races, client-only normal-path preservation, and L2.5
remain open gates. The predecessor receipt/value comparison exists, but its
behavior under real concurrent saves still needs evidence. G1 is not
...
recorded above. Before input capability promotion, reproduce actual
predecessor-effect versus same-key root-save/ETag-rebase races and confirm
that local changes are preserved or admission blocks. A value-identical
assignment is not itself evidence of a second durable writer; inspect whether
the normal patch is a no-op, conflicts, rebases, or commits. Close concurrent
chat edits, cross-tab admission, and joined input/result/owner/context
retention. Exercise the
